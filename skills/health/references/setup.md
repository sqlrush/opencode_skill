...
连接元数据复用 gdaa 的 `~/.gdaa/config.yaml`（无密码），口令经本机密钥 AES-256-GCM 加密存于 `~/.gdaa/credentials/*.enc`，由脚本解密；也可用环境变量 `GDAA_PASSWORD` 覆盖。`-c <NAME>` 用 `config.yaml` 里 `connections[].name` 字段。
...
| 退出码 2 / `connect ...` 报错 | 连不上（主机/端口/口令/库名错） | 核对 `~/.gdaa/config.yaml` 与网络；口令用 `GDAA_PASSWORD` 临时覆盖验证 |
