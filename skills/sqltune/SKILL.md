...
- **SQL 文本被截断时的回退。** 按 id 调用若报「SQL 被 openGauss 截断」（长 SQL 超过 `track_activity_query_size`，库里就没有完整文本，`gdaa` 同样如此），**不要**硬试——向用户索要完整 SQL，改用 `--sql-stdin` 传入完整文本走调优。若用户能调大 `track_activity_query_size` 并让该 SQL 重新执行，之后按 id 也能取全。
